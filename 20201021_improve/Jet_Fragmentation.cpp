...
//For Gluon    
    TH1D *Gluon_width = new TH1D("Gluon Fragmentation","Fragmentation function for Gluon" , 40 , 0, max_sigma) ;
...
//For Gluon
                       ...
                       if(min_pos >0 )
                       {
                           ...
                           for(;mindex < mindex_end;mindex++)
                           {
                                if(PFConstituents_charge->at(mindex) !=0)
                                {
                                    ...
                                    pt+=PFConstituents_pt->at(mindex);
                                    part_total_number++;
                                }
    ...
    c1->cd();
    Quark_width->Draw();
    c1->SaveAs("Fragmentation_quark.pdf");
    c1->Clear();
    c2->cd();
    Gluon_width->Draw();
    c2->SaveAs("Fragmentation_gluon.pdf");
    c2->Clear();
